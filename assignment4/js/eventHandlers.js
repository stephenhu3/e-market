...
$(document).ready(function() {
    $(".add-cart").click(function() {
        if (Object.keys(product).length > 0) {
            var productName = $(this).parents(".box").siblings(
                ".product-name").html();
    ...
    $(".remove-cart").click(function() {
        if (Object.keys(product).length > 0) {
            var displayName = $(this).parents(".box").siblings(
                ".product-name").html();
    ...
    $("body").on("click", ".add-quantity", function() {
        ...
        if (/cart-modal-quantity-row-\d+/.test(idString)) {
            ...
            var productName = $(".cart-product-name").children(
                "h5#cart-modal-name-row-" + relativeIndex).html();
            addToCart(productName);
            updateCartModal();
        }
    });
...
function updateCartModal() {
    ...
    var quantitiesHtml = "";
    for (var i = 0; i < cartItemList.length; i++) {
        quantitiesHtml += "<div><h5 class=\"in-cart-quantity\">" + cart[
                cartItemList[i]] + "</h5>" +
            "<button class=\"add-quantity\" + id=\"cart-modal-quantity-row-" +
            i +
            "\">+</button>" +
            "<button class=\"remove-quantity\" id=\"cart-modal-quantity-row-" +
            i +
            "\">-</button></div>";
    }
    ...
    $(".in-cart-quantity").each(function() {
        $(this).css("margin-left", this.innerHTML.toString().length * -
            8);
    });
    ...
    for (var i = 0; i < cartItemList.length; i++) {
        if (product[cartItemList[i]] !== undefined) {
            subtotalsHtml += "<h5>$" +
                cart[cartItemList[i]] * product[cartItemList[i]].price +
                "</h5>";
        }
    }
